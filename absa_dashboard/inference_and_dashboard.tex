...
Before visualization and analysis, raw review data undergoes a multi-stage preprocessing pipeline implemented in \texttt{preprocess\_data.py} and integrated into both the inference notebook and dashboard. The preprocessing ensures data quality, consistency, and compatibility with downstream analysis modules.

\paragraph{Data Loading and Format Detection.}
The pipeline supports multiple input formats (JSONL, Parquet, CSV) and automatically detects the text column by searching for common field names (\texttt{text}, \texttt{review}, \texttt{content}, \texttt{body}). For JSONL files, each line is parsed as a JSON object and converted to a DataFrame row. Parquet files are loaded using \texttt{pandas.read\_parquet} with schema inference.

\paragraph{Timestamp Normalization.}
Raw timestamps appear in multiple formats across datasets (ISO-8601 strings, Unix epoch integers, human-readable dates). The preprocessing module applies a cascading conversion strategy:
\begin{enumerate}
  \item Attempt \texttt{pd.to\_datetime} with \texttt{unit='ms'} for Unix timestamps (common in Amazon review datasets).
  \item If conversion fails, retry with \texttt{infer\_datetime\_format=True} for string formats.
  \item Coerce unparseable timestamps to \texttt{NaT} (Not-a-Time) and log warnings.
  \item Extract derived temporal features: \texttt{year}, \texttt{month}, \texttt{day\_of\_week}, \texttt{quarter} for temporal aggregations.
\end{enumerate}

\paragraph{Text Cleaning and Normalization.}
Review text fields undergo cleaning to remove noise and standardize formatting:
\begin{itemize}
  \item \textbf{Whitespace normalization}: Replace multiple spaces, tabs, and newlines with single spaces using regex \texttt{re.sub(r'\textbackslash s+', ' ', text)}.
  \item \textbf{HTML entity decoding}: Convert HTML entities (e.g., \texttt{\&amp;}, \texttt{\&lt;}, \texttt{\&quot;}) to Unicode characters using \texttt{html.unescape}.
  \item \textbf{Lowercasing (optional)}: For wordcloud and topic modeling, text is lowercased to ensure case-insensitive term aggregation. Original case is preserved in the \texttt{text} column for display.
  \item \textbf{Punctuation handling}: Punctuation is retained for ATEPC inference (model expects natural sentence structure) but stripped for keyword extraction.
\end{itemize}

\paragraph{Deduplication and Quality Filtering.}
To ensure data quality and avoid skewed statistics:
\begin{itemize}
  \item \textbf{Exact duplicate removal}: Reviews with identical \texttt{review\_id} and \texttt{text} are dropped using \texttt{df.drop\_duplicates(subset=['review\_id'])}.
  \item \textbf{Verified purchase filtering}: In the inference notebook, only reviews with \texttt{verified\_purchase=True} are processed to reduce spam and fake reviews (configurable via a flag).
  \item \textbf{Minimum length threshold}: Reviews shorter than 10 characters are filtered out as they typically lack meaningful aspects (e.g., single-word reviews like ``Great!'').
  \item \textbf{Missing value handling}: Rows with missing \texttt{text} or \texttt{rating} are dropped; missing \texttt{title} fields are filled with empty strings and concatenated with the main text during inference.
\end{itemize}

\paragraph{Rating and Metadata Validation.}
Numeric fields are validated and coerced to appropriate types:
\begin{itemize}
  \item \textbf{Rating normalization}: Ratings are cast to \texttt{float} and clipped to the range [1.0, 5.0]. Out-of-range values are logged and set to \texttt{NaN}.
  \item \textbf{Helpful votes parsing}: \texttt{helpful\_vote} fields (often stored as strings like ``5 out of 10'') are parsed using regex to extract numerator and denominator, then stored as separate columns.
  \item \textbf{ASIN validation}: Product identifiers (\texttt{parent\_asin}, \texttt{asin}) are validated against a regex pattern (alphanumeric, 10 characters) to catch malformed entries.
\end{itemize}

\paragraph{Feature Engineering for Dashboard.}
Additional derived features are computed to support filtering and visualization:
\begin{itemize}
  \item \texttt{review\_length}: Character count of the cleaned text, used for length distribution plots.
  \item \texttt{word\_count}: Token count after splitting on whitespace, used to filter overly short reviews.
  \item \texttt{has\_title}: Boolean flag indicating whether a review includes a title field (useful for filtering review completeness).
  \item \texttt{sentiment\_score}: Computed post-inference as $\frac{\text{Positive} - \text{Negative}}{\text{Total Aspects}}$, normalized to [-1, 1].
\end{itemize}

\paragraph{Output Format and Persistence.}
After preprocessing, the cleaned DataFrame is either:
\begin{itemize}
  \item Passed directly to the ATEPC inference loop (in \texttt{inference.ipynb}), or
  \item Cached to disk as an intermediate CSV (in the dashboard, using Streamlit's \texttt{@st.cache\_data}) to avoid redundant preprocessing on page refreshes.
\end{itemize}
Column order is standardized to: \texttt{review\_id}, \texttt{parent\_asin}, \texttt{rating}, \texttt{timestamp}, \texttt{text}, \texttt{title}, \texttt{verified\_purchase}, \texttt{user\_id}, ensuring compatibility with downstream modules.
