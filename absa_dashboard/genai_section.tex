...
\begin{itemize}
  ...
  \item Generator: OpenAI GPT (if \texttt{OPENAI\_API\_KEY} available) or local HF model (e.g., flan-t5-base).
...
\begin{itemize}
  ...
  \item Local Hugging Face pipeline (e.g., \texttt{google/flan-t5-base}) if OpenAI is not configured. The pipeline runs locally and is free to use (aside from machine resources). Model download occurs on first use; note that flan-t5-base is larger and slower on CPU.
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{LOCAL\_GEN\_MODEL} --- (optional) model name for local HF pipeline (default: \texttt{google/flan-t5-base}).
